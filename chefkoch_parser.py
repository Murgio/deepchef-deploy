...
class ChefkochParser:
    ...
    def food_list_html(self, fetch_from_internet=False):
        '''
            fetch_from_internet param: wheter to fetch the 
        ...
        if fetch_from_internet:
            ...
            for i, food in enumerate(self.result_list):
                ...
                try:
                    ...
                    pass
                result.append(self.generate_html(i, picture_url_big, picture_url_thumbnail, recipe_title, recipe_decs, ingredients_amount, ingredients_name, instructions, fetch_from_internet))
            return result
        else:
            for i, food in enumerate(self.result_list):
                try:
                    result.append(self.generate_html(*self.get_corresponding_recipes(i, food, fetch_from_internet)))
                except:
